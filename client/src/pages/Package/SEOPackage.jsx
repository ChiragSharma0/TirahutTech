import React, { useState } from "react";
import { motion } from "framer-motion";
import { Check, X } from "lucide-react";

const containerVariants = {
  hidden: { opacity: 0 },
  show: { opacity: 1, transition: { staggerChildren: 0.15 } },
};

const cardVariants = {
  hidden: { opacity: 0, y: 30 },
  show: {
    opacity: 1,
    y: 0,
    transition: { duration: 0.5, ease: "easeOut" },
  },
};

export default function SEOSection() {
  const [yearly, setYearly] = useState(false);

  const plans = [
    {
      name: "SEO Essential",
      price: yearly ? "$319 / yr" : "$399 / mo",
      gradient: "from-[#01686d] to-[#00444b]",
    },
    {
      name: "SEO Advanced",
      price: yearly ? "$559 / yr" : "$699 / mo",
      gradient: "from-[#f27b22] to-[#c85f12]",
      badge: "Best Seller",
    },
    {
      name: "SEO Professional",
      price: yearly ? "$799 / yr" : "$999 / mo",
      gradient: "from-[#01686d] to-[#00444b]",
    },
    {
      name: "SEO Enterprise",
      price: yearly ? "$1199 / yr" : "$1499 / mo",
      gradient: "from-[#f27b22] to-[#c85f12]",
    },
  ];

  const features = [
    { name: "Keywords", values: ["25", "40", "60", "100"] },
    { name: "Quality Backlinks (Monthly)", values: ["30", "50", "70", "100"] },
    { name: "Landing Pages", values: ["5", "10", "15", "30"] },
    { name: "G.E.O Optimization", values: [false, false, true, true] },
    { name: "A.E.O Optimization", values: [false, false, true, true] },
    { name: "Voice Search Optimization", values: [false, false, true, true] },
  ];

  return (
    <>
      {/* ================= HEADER (Services Style) ================= */}

      <motion.div
        initial={{ opacity: 0, y: -30 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.7 }}
        className="relative w-full py-28 sm:py-32 px-5 text-center overflow-hidden bg-[#e9f4f3]"
      >
        {/* Background Shapes */}

        <div className="absolute -top-32 -left-32 w-[380px] h-[180px] bg-[#00444b] rotate-45 opacity-90" />
        <div className="absolute top-24 left-40 w-[80px] h-[80px] bg-[#f27b22] rotate-45 shadow-lg" />

        <div className="absolute -bottom-36 -left-28 w-[380px] h-[180px] bg-[#01686d] rotate-45 opacity-90" />
        <div className="absolute bottom-24 left-16 w-[240px] h-[6px] bg-[#f27b22] rotate-45" />

        <div className="absolute -top-36 -right-36 w-[400px] h-[200px] bg-[#00444b] rotate-45 opacity-90" />
        <div className="absolute top-28 right-28 w-[200px] h-[6px] bg-[#f27b22] rotate-45" />

        <div className="absolute -bottom-24 right-24 w-[100px] h-[100px] bg-[#f27b22] rotate-45 shadow-lg" />

        {/* Glass Shapes */}

        <div className="absolute top-[35%] left-[30%] w-[200px] h-[200px] bg-[#01686d]/10 rotate-45 rounded-xl hidden sm:block" />
        <div className="absolute bottom-[25%] right-[35%] w-[220px] h-[220px] bg-[#00444b]/10 rotate-45 rounded-xl hidden sm:block" />

        {/* Content */}

        <div className="relative z-10 max-w-3xl mx-auto">

          <h1 className="text-3xl sm:text-4xl md:text-5xl font-bold text-[#00444b] mb-3">
            Our SEO Packages
          </h1>

          <p className="text-sm sm:text-base md:text-lg text-[#01686d] leading-relaxed">
            Powerful SEO strategies designed to increase traffic,
            improve ranking, and grow your business online.
          </p>

        </div>

      </motion.div>

      {/* ================= TOGGLE ================= */}

      <div className="flex justify-center items-center gap-4 mt-16">

        <span className="text-gray-600 font-medium">Monthly</span>

        <button
          onClick={() => setYearly(!yearly)}
          className={`w-14 h-7 rounded-full relative transition ${
            yearly ? "bg-[#01686d]" : "bg-gray-300"
          }`}
        >
          <span
            className={`absolute top-1 w-5 h-5 bg-white rounded-full transition ${
              yearly ? "right-1" : "left-1"
            }`}
          />
        </button>

        <span className="text-gray-600 font-medium">Yearly</span>

      </div>

      {/* ================= PRICING CARDS ================= */}

      <motion.div
        variants={containerVariants}
        initial="hidden"
        animate="show"
        className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 max-w-7xl mx-auto mt-16 px-4"
      >

        {plans.map((plan, i) => (
          <motion.div
            key={i}
            variants={cardVariants}
            whileHover={{ y: -10, scale: 1.03 }}
            className="relative rounded-3xl p-[1.5px] bg-gradient-to-br from-[#01686d] via-[#f27b22] to-[#00444b]"
          >

            <div className="bg-white rounded-3xl p-8 h-full">

              {plan.badge && (
                <span className="absolute -top-4 left-1/2 -translate-x-1/2
                 bg-[#f27b22] text-white text-xs font-bold
                 px-4 py-1 rounded-full shadow-lg">
                  {plan.badge}
                </span>
              )}

              <div className={`rounded-2xl text-white text-center py-8 mb-8 bg-gradient-to-r ${plan.gradient}`}>

                <h2 className="text-lg font-semibold">{plan.name}</h2>

                <p className="text-4xl font-extrabold mt-2">{plan.price}</p>

              </div>

              <ul className="text-sm text-gray-600 space-y-2 mb-8">

                <li>✔ Keyword Optimization</li>
                <li>✔ Technical SEO</li>
                <li>✔ On Page SEO</li>
                <li>✔ Monthly Reports</li>

              </ul>

              <button
                className="w-full py-3 rounded-xl font-semibold text-white
                bg-gradient-to-r from-[#01686d] to-[#00444b]
                hover:from-[#00444b] hover:to-[#01686d]
                transition shadow-lg"
              >
                Get Started
              </button>

            </div>

          </motion.div>
        ))}

      </motion.div>

      {/* ================= FEATURES TABLE ================= */}

      <div className="max-w-7xl mx-auto mt-24 mb-24 bg-white rounded-2xl border shadow-xl overflow-x-auto">

        <div className="bg-gradient-to-r from-[#01686d] to-[#00444b]
        text-white px-6 py-4 font-bold text-lg flex justify-between rounded-t-2xl">
          <span>SEO Features Breakdown</span>
          <span className="text-2xl">+</span>
        </div>

        <table className="w-full min-w-[1100px] border-collapse text-sm">

          <thead className="bg-gray-50 font-bold">
            <tr>
              <th className="border p-4 text-left">Feature</th>
              <th className="border p-4">Essential</th>
              <th className="border p-4">Advanced</th>
              <th className="border p-4">Professional</th>
              <th className="border p-4">Enterprise</th>
            </tr>
          </thead>

          <tbody>

            {features.map((feature, i) => (
              <tr key={i} className="odd:bg-gray-50">

                <td className="border px-4 py-3 font-medium text-[#00444b]">
                  {feature.name}
                </td>

                {feature.values.map((val, idx) => (
                  <td key={idx} className="border px-4 py-3 text-center">

                    {typeof val === "boolean" ? (
                      val ? (
                        <Check className="mx-auto text-[#01686d]" />
                      ) : (
                        <X className="mx-auto text-red-500" />
                      )
                    ) : (
                      <span className="font-semibold">{val}</span>
                    )}

                  </td>
                ))}

              </tr>
            ))}

          </tbody>

        </table>

      </div>

      {/* Bottom Gradient */}

      <div className="w-full h-2 bg-gradient-to-r from-[#01686d] via-[#f27b22] to-[#00444b]" />

    </>
  );
}